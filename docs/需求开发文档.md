...
## 11. 生产数据库迁移记录（2026-02-12）

问题背景：

1. 线上最初使用 `file:/tmp/dev.db`（临时 SQLite），导致实例切换后出现 `main.users does not exist`。

迁移动作：

1. 接入 Neon Postgres（持久化）。
2. Prisma 数据源从 `sqlite` 切换为 `postgresql`：
- `prisma/schema.prisma`
3. 使用 `prisma db push` 将表结构推送到 Neon。
4. Vercel 生产 `DATABASE_URL` 切换到 Neon 连接串。
5. 在 `src/lib/prisma.ts` 增加 `DATABASE_URL` 运行时 `trim`，规避平台注入尾部换行。

结果：

1. 线上不再依赖临时文件数据库。
2. 用户/角色/会话数据可持久保存，重部署不丢表结构。
